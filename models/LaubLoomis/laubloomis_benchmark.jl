...
W = 0.01
prob = laubloomis(W=W)
alg = TMJets(abstol=5e-11, orderT=7, orderQ=1)
...
W = 0.05
prob = laubloomis(W=W)
alg = TMJets(abstol=5e-4, orderT=3, orderQ=1)
...
W = 0.1
prob = laubloomis(W=W)
alg = TMJets(abstol=3e-4, orderT=3, orderQ=1)
...
for (i, c) in enumerate(cases)
    print(io, "$model,$c,$(validation[i]),$(runtimes[c]),$(final_width[i])\n")
end
